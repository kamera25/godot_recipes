...
オブジェクトが破壊されたり収集された際に再生するために {{< gd-icon AudioStreamPlayer >}}`AudioStreamPlayer` をモブ/コインなどに追加しました。ただ、対象のオブジェクトを削除すると、オーディオプレイヤーも同時に削除され、音が途切れる問題が発生します。より簡単に音声を管理できる方法を教えてください。

## 解決策

この問題を解決するため、SceneTreeのどこからでもアクセス可能なノードを使用します。このノードは一連の{{< gd-icon AudioStreamPlayer >}}`AudioStreamPlayer`ノードと、再生するサウンドストリームのキューを管理します。
