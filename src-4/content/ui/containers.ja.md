...
解決策は `コンテナ` コンポーネントです。コンテナを活用することで、UIのレイアウト構築が大幅に効率化されます。

`コントロール`ノード（`{{< gd-icon Control >}}`）を`コンテナ`に追加すると、コンテナが当該コントロールの位置指定情報をすべて継承します。これにより、子要素のサイズや位置、その他のレイアウトプロパティを個別に設定することはできなくなります。
...
* {{< gd-icon CenterContainer >}} `センターコンテナー`

    このコンテナは子要素を中央に配置します。

* `{{< gd-icon MarginContainer >}}`MarginContainer`

    このコンテナはマージンを保持しており、子要素がコンテナの端に近づきすぎるのを防ぎます。マージン値はプロパティの「カスタム定数」セクションで設定可能です。

* {{< gd-icon VBoxContainer >}} ``VBoxContainer`` / {{< gd-icon HBoxContainer >}} ``HBoxContainer``

    これらのコンテナは、内容物を垂直または水平方向に整列させます。「カスタム定数」セクションでは、要素間の間隔を広げるための*分離*プロパティも設定可能です。
...
* 中心縮小
    ...
    「＊フィル」が無効で「＊拡張」が有効になっている場合、コントロールはエリアの先頭ではなく、中央に配置されたままになります。
...
- [ラベル](/godot_recipes/3.x/ui/labels)
