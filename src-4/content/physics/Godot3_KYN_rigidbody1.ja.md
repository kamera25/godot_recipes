...
<a href="https://docs.godotengine.org/ja/latest/classes/class_rigidbody2d.html" target="_blank"><svg width="18" height="18" class="icon-icon_rigid_body_2d">
...
![Godot 4: リジッドボディを使用 (rigidbody properties)](/godot_recipes/4.x/img/rigidbody_properties.png)

The body's behavior is also affected by the world, via the _Project Settings -> Physics_
properties, or by entering an <a href="https://docs.godotengine.org/ja/latest/classes/class_area2d.html"><svg width="18" height="18" class="icon-icon_area_2d" target="_blank"><use xlink:href="/blog/img/symbol-defs.svg#icon-icon_area_2d"></svg> `Area2D`</a> that is overriding the global physics properties.
...
Start by creating a RigidBody2D for the block and adding <a href="https://docs.godotengine.org/ja/latest/classes/class_sprite.html" target="_blank"><svg width="18" height="18" class="icon-icon_sprite"><use xlink:href="/blog/img/symbol-defs.svg#icon-icon_sprite"></svg>`Sprite`</a> and
<a href="https://docs.godotengine.org/ja/latest/classes/class_collisionshape2D.html" target="_blank"><svg width="18" height="18" class="icon-icon_collision_shape_2d"><use xlink:href="/blog/img/symbol-defs.svg#icon-icon_collision_shape_2d"></svg>`CollisionShape2D`</a>
children:

![Godot 4: リジッドボディを使用 (rigidbody block scene)](/godot_recipes/4.x/img/rigidbody_block_scene.png)
...
![Godot 4: リジッドボディを使用 (rigidbody scene1)](/godot_recipes/4.x/img/rigidbody_scene1.png)
...
![Godot 4: リジッドボディを使用 (rigidbody vel)](/godot_recipes/4.x/img/rigidbody_vel.gif)
...
![Godot 4: リジッドボディを使用 (rigidbody impulse)](/godot_recipes/4.x/img/rigidbody_impulse.gif)
...
![Godot 4: リジッドボディを使用 (ship red)](/godot_recipes/4.x/img/ship_red.png)
...
> 注意：Godot 3.0では、`0` 度は右方向（**x** 軸に沿って）を指します。
> このため、`Sprite` に `Rotation` を 90度追加しましょう。そうすることで
> `Sprite` の向きが物体の方向と一致するようになります。
...
このスクリプトの動作を順を追って説明しましょう。変数 `engine_thrust` と
`spin_thrust` は、船の加速速度と旋回性能を決定します。インスペクターでそれぞれ
`500` と `25000` に設定しましょう（トルク単位のため数値が大きくなります）。`thrust` は船のエンジン状態を表します：惰行時は `(0, 0)`、動力がオンの場合は `engine_thrust` の長さを持つベクトルになります。
`rotation_dir` は船が旋回する方向を示します。変数 `screensize` はスクリーンサイズを保持し、後で使用します。
...
最後に、物理関連の処理は`_physics_process()`内で行いましょう。ここではまず、船が向いている`方向`に沿って`推力`を適用するために`set_applied_force()`を使用しています。その後、`set_applied_torque()`を使用して船を回転させます。

シーンを再生しましょう - 自由に空を飛べるはずです。

![Godot 4: リジッドボディを使用 (rigidbody ship1)](/godot_recipes/4.x/img/rigidbody_ship1.gif)
...
[RigidBody2Dドキュメント](https://docs.godotengine.org/ja/latest/classes/class_rigidbody2d.html#description)より引用すると：

> リジッドボディの位置や直線速度をフレームごとに、あるいは頻繁に変更することは避けるべきです。状態に直接影響を与えたい場合は、物理演算の状態を直接操作できる `_integrate_forces` メソッドを使いましょう。

[_integrate_forces の説明](https://docs.godotengine.org/ja/latest/classes/class_rigidbody2d.html#class-rigidbody2d-integrate-forces)：
...
![Godot 4: リジッドボディを使用 (rigidbody ship2)](/godot_recipes/4.x/img/rigidbody_ship2.gif)
