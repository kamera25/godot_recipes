...
必ずスプライトを正しく配置します。回転していないオブジェクトは **+X軸** 方向に向くようにしましょう（つまり右方向を向いている状態）。スプライトのアートワークが別の方向を向いて描かれている場合は、 {{< gd-icon Sprite2D >}}`Sprite2D`（親ボディではなく）を適切に整列させるために回転させてください。
{{% /notice %}}
