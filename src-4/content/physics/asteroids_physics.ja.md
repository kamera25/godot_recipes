...
必ずスプライトを正しく配置します。回転していないオブジェクトは **+X軸** 方向に向くようにしてください（つまり右方向を向いている状態）。スプライトのアートワークが別の方向を向いて描かれている場合は、 {{< gd-icon Sprite2D >}}`Sprite2D`（親ボディではなく）を適切に整列させるために回転させてください。
{{% /notice %}}
...
`thrust` と `rotation_dir` は入力操作によって設定されます。次にその方法を確認します。
