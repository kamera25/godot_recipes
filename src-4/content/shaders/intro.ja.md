...
シェーダープログラムの出力結果は、オブジェクトを構成するピクセル群の色情報です。シェーダーは2D環境（`canvas_item` 用シェーダー）および3D環境（`spatial` シェーダー）で利用可能です。
...
シェーダーの最初の行にはそのタイプを指定する必要があります。接続されているノードが2Dノードの場合は：
...
これらの初期例では2Dに限定して進めましょう。まず`Sprite`ノードを追加し、上記の手順に従ってシェーダーを適用してください。テクスチャにはGodotのデフォルトアイコンを使用することも可能です。

ここで解説するシェーダーには主に2種類あります：_頂点シェーダー_と_フラグメントシェーダー_です。
...
<img src="/godot_recipes/3.x/img/shader_intro_01.png" alt="画像">

每個像素都是紅色。`COLOR`是片元程序的輸出，同時被應用於所有像素上。但如果我們希望有一些變化呢？
...
{{% notice note %}}
シェーダーではベクトル型 (`vec4`) を使用してRGBAカラーを表現します。個々の成分には `color.r` のようにアクセス可能です。色をベクトルとして扱うことで、ベクトル演算に基づく様々な興味深いエフェクトを実現できます。
{{% /notice %}}
...
<img src="/godot_recipes/3.x/img/shader_intro_04.gif" alt="シェーダー入門 04">
...
インスペクターで値を設定する際に補助として使用できるオプションの＊ヒント＊も利用可能です。
...
これはシェーダーで実現できる機能のほんの一例に過ぎません。このセクションの他のレシピも参照して、ご自身のプロジェクトで使えるより多様な事例をご覧ください。
