...
シェーダーに値を渡すには、`_uniform_`キーワードで宣言された変数が必要です。この設定を行うと、インスペクタには`export`変数と似た形でその変数が表示されます。ただし注意点として、ユニフォーム変数の値はシェーダー内で*変更することはできません*！

均一値（uniforms）はシェーダー全体でグローバルに使用可能であり、任意の関数からアクセスできます。

### ヒント

インスペクタで値を設定する際に補助として使用できるオプションの *ヒント* も利用できます。
