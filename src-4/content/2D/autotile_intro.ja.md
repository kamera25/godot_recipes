...
{{< gd-icon TileMap >}} TileMapを使用し、自動タイル機能を活用してレベル作成を効率化したいです。
...
最後に、「アイコン」ボタンをクリックして、自動タイルグループのアイコンとして使用するタイルを選択します。これが描画中にカーソル下に表示されるタイルとなります。

タイルマップに切り替えてください。正常に描画できるはずです。問題がある場合は、ビットマスクを確認し、すべてのタイルに空白部分が残っていないか確認します。上記の画像と照合して確認します。
