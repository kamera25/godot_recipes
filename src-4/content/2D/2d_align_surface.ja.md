...
キャラクターボディは、立っている面と平行になるように調整が必要です。

## 作り方

まずは基本的なプラットフォームキャラクターから始めてください。詳細は[プラットフォームキャラクター](/godot_recipes/4.x/ja/2d/platform_char/)を参照します。
...
![Godot 4: CharacterBody2D：表面と整列 (2d align 04)](/godot_recipes/4.x/img/2d_align_04.png)

移動時には、`x` 方向の速度をローカル座標系の `x軸`（赤い矢印）と一致させ、重力/ジャンプ動作をローカル `y軸`（緑の矢印）と一致させましょう。入力処理コードはそのままで、常に `velocity` がローカル座標系で計算されると仮定できます。唯一の問題は、`move_and_slide()` メソッドが速度ベクトルをグローバル座標系で受け取ることを期待している点です。この問題を解決するため、`move_and_slide_with_snap()` 関数を調整しましょう。
