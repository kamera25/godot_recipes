...
![Godot 4: Screen Shake (shake rand01)](/godot_recipes/3.x/img/shake_rand01.gif)
...
Noise, or more specifically *gradient noise* is a method for generating more natural-looking "random" patterns. To do this, Godot provides the [FastNoiseLite](https://docs.godotengine.org/en/latest/classes/class_opensimplexnoise.html) class.
...
`FastNoiseLite` works by generating a 3D "cloud" of points. Each point has a value ranging from `-1` to `1`. Below are two examples of noise generated by `FastNoiseLite`. In the images, each pixel's white value is mapped to the noise value at that point.

![Godot 4: Screen Shake (2d noise example)](/godot_recipes/3.x/img/2d_noise_example.png)

As you can see, while "noisy", if you pick a particular pixel, adjacent pixels may vary unpredictably, but won't suddenly swing wildly from one extreme to another. The behavior can be adjusted to suit your needs, but we won't go into the details of configuring FastNoiseLite here. See the [related recipes](#related-recipes) section below for links to more information.
...
![Godot 4: Screen Shake (shake noise01)](/godot_recipes/3.x/img/shake_noise01.gif)
