...
またここでは、{{< gd-icon CharacterBody2D >}}`CharacterBody2D` ノードを使用していると想定します。
...
{{% notice note %}}
Godotでは角度 `0` 度を`x`軸に沿っていることを意味します。これは、ノードの前方方向（`transform.x`）が右向きであることを示しています。キャラクタースプライトも同様に、右側に向かって描画されるように設定してください。
{{% /notice %}}
