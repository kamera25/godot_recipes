...
このデモでは、ローカルマルチプレイヤーゲーム――トップダウンスタイルの迷路ゲームを例に挙げましょう。このゲームでは2人のプレイヤーが参加し、一方は矢印キー、もう一方はWASDキーで操作します。これは問題ありませんが、もしゲーム世界全体が1画面に収められる程度の大きさであれば、特に問題はありません。しかし、マップが非常に広い場合、両プレイヤーを個別に表示する「分割画面」ビューが必要になるでしょう。
...
各操作は、プロジェクト設定の[入力マップ]セクションで個別に設定されています：「right_1」は右矢印キー、「right_2」はDキーなどです。このように命名することで、コード内で以下の構文を使用でき、開発効率を大幅に向上させられます：
...
<img src="/godot_recipes/3.x/img/splitscreen_map.png" alt="Split Screen Map">
...
以下のように、「World」シーンを「Viewport1」の子としてインスタンス化しましょう。これでシーンを実行すると、左ビューポート内に世界が表示されます。

また、「ビューポート2」に世界を追加する必要がありますが、同じ世界を使用させたい場合はどうすればよいでしょうか。
これはコードで処理できます。メインオブジェクトにスクリプトを添付し、以下を追加してください。
...
次に、プレイヤーカメラがマップの表示範囲外にスクロールしないように制限を追加しましょう。この関数をメインスクリプトに追加し、`_ready()` で呼び出してください。
...

{
  "steps": [
    {
      "description": "新たに `ViewportContainer` 要素を作成する必要がありますが、今回は `Main` の子要素として追加します。今回のケースでは**Stretchモードは使用しません**。以下の手順に従ってください。\
...
public function _ready() {
    ...
    $this->minimap->setWorldViewportIndex(0); // ミニマップのワールドビューポートインデックスを設定
}
...
私たちはエッジ周りのグレーゾーンを解消する必要があります。正確なズームレベルを特定して希望のミニマップサイズに合わせることも可能ですが、代わりに{{< gd-icon Viewport >}}`Viewport`設定の_透過背景(Bg_をチェックします。これで非地図領域が見えなくなり、ミニマップがメインビューポートの上に直接浮かんで表示されるようになります。
