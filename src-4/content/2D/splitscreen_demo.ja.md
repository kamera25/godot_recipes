...
{{< gd-icon Viewport >}}`Viewport`にコンテンツを表示するには、{{< gd-icon Camera2D >}}`Camera2D`が必要です。このカメラは{{< gd-icon Viewport >}}`Viewport`上にレンダリングを行います。各ビューポートに1つずつ追加します。また、カメラを有効にするには _現在のプロパティを必ずチェックしてください_ 。さらに、各カメラのズーム値を`(0.75, 0.75)`に設定することで、プレイヤー周辺のエリアをより詳細に表示できるようになります。
