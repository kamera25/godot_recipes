...
例えば、見下ろし型キャラクターのゲームを開発していて、`入力ActionKey`を使って矢印キーで移動操作するコードを書いたとします。するとすぐに、多くのプレイヤーが「WASD」スタイルの操作方法を好むことに気づくでしょう。後からコードに戻って追加のキーチェックを実装することもできますが、これでは重複した冗長なコードが生じてしまいます。
