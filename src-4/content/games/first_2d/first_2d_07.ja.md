...
現在のキーフレームを時間 `0` 時点で`0`に、時間 `0.4` 時点で`5`に設定してください。アニメーションを実行して動作を確認してください。
...
それでは、`Main` シーンに移動して、これらの敵キャラクターをゲームに追加してください。`Main`シーンにスクリプトを追加し、まず敵キャラ用シーンを読み込むことから始めます。
