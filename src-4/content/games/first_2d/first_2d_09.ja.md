...
右側にはスコアを表示したいと考えています。単に{{< gd-icon Label >}}`Label`ノードを使ってフォントを追加するだけでもできますが、それでは面白みがありません。アートパックには、代わりに使用できる美しいピクセル数字セットが含まれています。適切に表示するためには、少しコーディングして数値を切り出す必要があります。
...
また、シールドをプレイヤースクリプトに追加することも可能です。`player.gd`ファイルの先頭にこれらの新しい行を追加してください：
