...
右側にはスコアを表示したいと考えています。単に{{< gd-icon ラベル >}}`ラベル`ノードを使ってフォントを追加するだけでもできますが、それでは面白みがありません。アートパックには、代わりに使用できる美しいピクセル数字セットが含まれています。適切に表示するためには、少しコーディングして数値を切り出す必要があります。
