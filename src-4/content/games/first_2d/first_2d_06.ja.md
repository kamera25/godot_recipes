...
We can make the scene more dynamic by animating the background. While we could do this in code by changing the `region_rect` property every frame, we'll use an {{< gd-icon アニメーションPlayer >}}`アニメーションPlayer` node instead; add one as a child of `Main`.
