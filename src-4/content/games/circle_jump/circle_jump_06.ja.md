...
スクリプトを 自動読み込み に登録するには、「プロジェクト設定」を開き、「自動読み込み」タブを選択します。ロードしたいスクリプトのフォルダをクリックし、その後「追加」をクリックします。

![Godot 4: サウンドとカラー設定 (cj 06 01)](/godot_recipes/4.x/img/cj_06_01.png)

## サウンドの追加方法

サウンドを再生するために、複数の`AudioStreamPlayer`ノードを異なるシーンに追加します。

- まず `Main` シーンに「Music」という名前を追加します。_Stream_ プロパティには `res://assets/audio/Music_Light-Puzzles.ogg` を使いましょう。

- `Screens` シーンに新たに「Click(クリック)」という画面を追加します。ボタンをタップした際にこの画面が表示されるようにします。アセットフォルダから`menu_click.wav`を使用します。
...
サウンドを再生するには、それぞれの `play()` メソッドを呼び出すだけです。`Main.new_game()` にこれを追加します。
...
`Screens.gd` ファイルの `_on_button_pressed()` 関数に以下を追加します。
