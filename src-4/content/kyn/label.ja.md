...
{{< gd-icon 标签 >}} の `ラベル` は、フォーマットされていないテキストを表示するための {{< gd-icon Control >}}`コントロール`ノードであり、テキストの配置や折り返しなどの制御オプションを備えています。
...
{{< gd-icon 标签 >}}の「カスタムフォント」セクションにて、「新規ダイナミックフォント」を選択してください。`ダイナミックフォント`は、指定された書体でテキストを表示するリソース型です。

追加した「DynamicFont」を選択し、「フォント/フォントデータ」で「ロード」を選び、フォントファイルを選択します。また、フォントの*サイズ*も設定してください。