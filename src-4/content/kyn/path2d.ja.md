...
*パス* とは 2D 空間における点の連続であり、{{< gd-icon Curve2D >}}`Curve2D` リソースによって定義されます。{{< gd-icon Path2D >}}`Path2D` ノードを使用すると、2D 空間でパスを配置できるほか、エディタ内で新規パスを作成することも可能です。
...
「ポイントを追加」を選択し、エディタウィンドウ内でクリックしてポイントを作成します。
...
単体では{{< gd-icon Path2D >}} `Path2D`ノードにはほとんど機能がありません。経路に沿って移動するには、{{< gd-icon PathFollow2D >}} `PathFollow2D` ノードも併せて使用する必要があります。これは親の {{< gd-icon Path2D >}} `Path2D` に沿って移動する役割を持つノードです。
