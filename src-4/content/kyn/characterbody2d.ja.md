...
このメソッドには `distance` というパラメーターが必要です。これは、指定されたフレーム内で対象物体を移動させたいベクトルを指定します。通常は `velocity` ベクトルに `delta` を掛けたものを渡します（つまり、距離に相当する値です）。
...
詳細については[プラットフォームキャラクター]レシピを参照しましょう（/godot_recipes/4.x/ja/2d/platform_character/）。
