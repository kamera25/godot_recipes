...
{{% notice warning %}}
{{< gd-icon RigidBody2D >}}`RigidBody2D`の物理的特性（例：`position`や`linear_velocity`）を直接設定しようとしても正常に動作しません。これらの値は物理エンジンによって管理されています。
{{% /notice %}}

また、プロジェクト設定の［物理プロパティ］やグローバル物理プロパティを上書きする {{< gd-icon Area2D >}}`Area2D` オブジェクトに入ることで、ボディの挙動は外部環境にも影響されます。
...
1. 動的モード - このモードではオブジェクトは{{< gd-icon CharacterBody2D >}}`CharacterBody2D`のように振る舞い、コードによってのみ移動させることができます。**注意:** これは必ずしも{{< gd-icon CharacterBody2D >}}`CharacterBody2D`の補助関数（例：`move_and_slide()`）が使えるという意味ではありません。すべての移動と衝突応答は手動で実装しましょう。
...
デフォルトでは、{{< gd-icon RigidBody2D >}}`RigidBody2D` オブジェクトが他の空間内の物体と衝突・相互作用しても、その衝突は報告されません。
...
![Godot 4: RigidBody2D (rigidbody ship1)](/godot_recipes/4.x/img/rigidbody_ship1.gif)
...
![Godot 4: RigidBody2D (rigidbody ship2)](/godot_recipes/4.x/img/rigidbody_ship2.gif)
