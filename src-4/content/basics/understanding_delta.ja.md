...
問題を具体的に説明するため、画面上を移動する「スプライト」ノードを考えてみます。画面幅が600ピクセルで、スプライトがこのスクリーン全体を横切るのに5秒かかる場合、必要な移動速度は以下の計算で求められます。
...
`_process()`関数を使用する場合、自動的にエンジンから渡されるパラメーターとして`delta`が含まれます（これは`_physics_process()`と同様で、物理関連コード用に使います）。これは時間間隔を表す浮動小数点数値であり、直前のフレームから経過した時間の長さを示します。通常この値は1/60秒、つまり約0.0167秒に相当します。
