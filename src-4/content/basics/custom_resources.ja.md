...
* ゲーム画面にはHPゲージを表示し、発生する変化を適切に表示しましょう
...
まず最初に、新しいカスタムリソース`PlayerHealth`を定義しましょう。このリソースはHPに関連するプロパティを管理しましょう。さらに、HPの変化（回復やダメージを受けるなど）を処理するための機能とシグナルを提供します。

_Script_ タブで、 _ファイル >新規スクリプト_ を選択します。`Resource`を継承するように設定し、ファイル名を `PlayerHealth.gd` とします。
...
* プレイヤー： {{< gd-icon CharacterBody2D >}}`CharacterBody2D` コンポーネントを搭載したキャラクターモデル
* UI： 体力ゲージを表示する {{< gd-icon TextureProgressBar >}}`ProgressTexture` を含むインターフェースシステム
* 回復ゾーン： その上を通過する対象を回復する {{< gd-icon Area2D >}}`Area2D` オブジェクト
* スパイクタイル： 接触時にダメージを与える {{< gd-icon TileMap >}}`TileMap` 要素のタイルセット
