...
3.x系バージョンに慣れているユーザーにとって最も大きな変化は、`playing`プロパティが廃止された点です。現在は{{< gd-icon アニメーションPlayer >}}`アニメーションPlayer`との整合性が大幅に改善されており、アニメーションを自動再生するには**SpriteFrames**パネルでオートプレイ機能を有効にするだけで済みます。コード側では`play()`メソッドと`stop()`メソッドを使用して、再生制御する形式に変更されました。
