...
アニメーション状態機械を作成するには {{< gd-icon AnimationTree >}}`AnimationTree` を使いましょう。これにより、アニメーションを整理できるだけでなく、最も重要な点として、それらの間の遷移を制御することが可能になります。
...
シーンに {{< gd-icon AnimationTree >}}`AnimationTree` ノードを追加してください。［ツリールート］プロパティで「新規アニメーションノードステートマシン」を選択してください。
