...
まず、シーンに地形を追加しました。こちらの[リンクからダウンロード](https://fertile-soil-productions.itch.io/modular-terrain-pack)できます。ローポリモデルですが、お好みの地形を自由に使用・作成していただいて構いません。
...
その代わりに、戦車の履帯が地面と平行になるように車体を傾けましょう。勾配が変わっても姿勢を維持しなければなりません。そのためには、どの方向が「上」かを把握しておく必要があるのです。
...
問題は、タンクの衝突判定形状が地形面のうち複数箇所と干渉している可能性があることです。さらに、`move_and_slide()` 関数では1フレーム内で複数箇所で衝突が発生する場合があり、これが画面表示のカクつき（ジャダー）を引き起こしています。この問題を解決するためには、単一の面を選択し、一貫してその面で判定しましょう。
