...
キューブのテクスチャが非対称の場合、転がすたびにリセットされることに気付くかもしれません。メッシュの回転を保持するには、以下を追加します。
...
{{% notice note %}}
以下の方法も使用できます。{{< gd-icon RayCast3D >}}`RayCast3D`ノードを使用する場合。ただし、チェックを実行する前に必ず`force_raycast_update()`を呼び出すようにします。
{{% /notice %}}
