...
まず、地上走行時と空中飛行時を明確に区別が必要です。地上では速度を0に減速できますが、空中では最低限の対気速度を維持しなければなりません。また、地上走行中は旋回時にバンク（傾斜）しないようが必要です（翼が地面に接触して損傷する危険があるため）。
...
このテクニックは、様々なアーケードスタイルの飛行ゲームに応用できます。例えば、マウス操作の場合、`入力EventMouseMotion` の `relative`プロパティを使用してピッチとヨー入力を設定する方法が有効です。
