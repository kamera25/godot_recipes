...
それでは、必要な変数から始めつつ、スクリプトを追加してください。
...
「これが動作中の戦車です。テスト用シーンとして、地面には {{< gd-icon StaticBody3D >}} 球体、カメラには [補間付きカメラ](/godot_recipes/4.x/ja/3d/interpolated_camera/index.html) レシピを使用した `Camera3D` を配置しています。
