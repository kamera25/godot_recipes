...
シーンに `アニメーションTree` コンポーネントを追加します。［インスペクタ］で［ツリールート］を新規作成された `アニメーションNodeStateMachine` に設定し、［Animプレイヤー］ではキャラクターのアニメーションノードを選択します（{{< gd-icon アニメーションPlayer >}}`アニメーションPlayer`）。最後に［有効化］チェックボックスをオンにしてください。
...
遷移条件を変更するには、アイコンを使用して**選択モード**に切り替えた後、いずれかの接続をクリックしてください。インスペクタウィンドウで接続プロパティを確認できます。アイドル状態からアタック状態への遷移では、**進行/モード** を「有効」（"自動" ではなく）に設定したいです。つまり、指示があった場合にのみ実行されるということです。接続ライン上のアイコンの色が変化することに注意してください。
