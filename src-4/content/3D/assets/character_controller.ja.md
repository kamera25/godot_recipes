...
Godot の 3D 操作に慣れている方なら、キャラクターが画面後方の **+Z** 方向に向けていることにもお気づきでしょう。{{< gd-icon `Skeleton3D` >}} ノードを選択し、**Y軸回転**を `180` 度に変更して修正します。
...
ルートノードの子として追加し、その後に {{< gd-icon Camera3D >}}`Camera3D` をその子要素として追加します。
...
その前に、これは機能をテストする絶好の機会です。地面に大きな {{< gd-icon StaticBody3D >}}`StaticBody3D` を使ったクイックテストシーンを作成するか、ダンジョンパックアセットを使用してシーン制作を開始します。
...
現在は、キャラクターを回転させて、移動方向を向くようにさせましょう。
...
次に、`_physics_process()` 関数の `move_and_slide()` 呼び出し後に以下を追加します。
...
`velocity` はグローバル座標系ですが、キャラクターモデルが回転しているため、モデルの `basis` を使用して `velocity` をローカル座標系に変換しましょう。変換後、3Dベクトルをブレンドスペースの2Dベクトルにマッピングし、`speed` で割ることで `-1` から `1` の範囲の値を得ます。また、*-z*方向は前方ですが、*+y*方向はブレンドスペースにおける前進アニメーションを表すため、値が一致するよう符号を調整します。

注意: このパラメーターパスは、{{< gd-icon AnimationTree >}}`AnimationTree`インスペクターを確認することで取得できます。実際にスクリプトウィンドウにドラッグ＆ドロップして入力することもできます。

### 攻撃方法

攻撃動作については、まず`「attack」`という入力アクションを追加します。このコマンドは左マウスボタンに割り当てています。
...
次に、地面に接地したタイミングを把握しましょう。これにより、「Jump_Idle」アニメーションから遷移できます。これを実現するためには、前フレームと比較することで接地状態を追跡しましょう。上部に新しい変数を追加します。
