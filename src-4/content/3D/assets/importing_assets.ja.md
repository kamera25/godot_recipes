...
プロジェクトにファイルをコピーする前に、以下の点に注意してください。アセットにはOBJ、FBX、GLTFといった複数の異なるファイル形式があります。また、変更を希望する場合に備えてサンプルや個別テクスチャなどの追加ファイルも用意されています。これらすべては不要であり、GodotではGLTF形式が最も推奨されるインポートフォーマットです。したがって、必ず`gltf`フォルダまたは`.gltf`ファイル（もしくは同等のバイナリ形式である`.glb`）のみをプロジェクトディレクトリにドラッグ＆ドロップしてください。
...
プレイヤーが {{< gd-icon CharacterBody3D >}}`CharacterBody3D`としてコード化されるため、ここでノードタイプを指定できます。`シーンルート`をクリックし、右側のパネルで**ルートの型**を {{< gd-icon CharacterBody3D >}}`CharacterBody3D`に設定してください。
