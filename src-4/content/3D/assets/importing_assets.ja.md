...
このシーンではすべてのモデルと［アニメーションプレーヤー］{{< gd-icon アニメーションPlayer >}} `アニメーションPlayer` が表示され、ここではアニメーションを実際にテストすることができます。
...
特定タイプのノードすべてに適用可能な*インポートスクリプト*を作成することができます。例えば、前述した静的衝突判定オブジェクトの自動生成を自動化できます。
