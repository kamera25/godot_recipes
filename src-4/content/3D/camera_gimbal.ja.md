...
以下の手順を試してみてください。{{< gd-icon Camera3D >}}`Camera3D`ノードを1つ取得し、**X軸**（ギズモ上の赤いリング）を中心にわずかに回転させた後、続いて**Z軸**（青いリング）を中心に少し回転させます。次に、**X軸**方向の回転を逆方向に反転させ、「プレビュー」ボタンをクリックしてください。カメラが斜めに傾いた状態になっていることが確認できるはずです。
...
{{< gd-icon Camera3D >}}`Camera3D` の **Transform/Position** を `(0, 0, 4)` に設定してください。

以下にジンバルの仕組みを説明します。外側ノードは**Y軸方向のみ**回転可能で、内側ノードは**X軸方向のみ**回転できます。手動で試したい場合は、まず「ローカル空間を使用」（メニューバーのロックアイコン隣にあるキューブアイコン - ショートカットキーは「T」）に切り替えることを忘れないでください。必ず外側ノードの _緑色リング_ のみを、内側ノードの _赤色リング_ のみを操作してください。カメラノードには一切触れないよう注意してください。
...
キーボード操作から始めてください。その後、マウスを使用するオプションも追加してください。必要な操作と割り当てられた入力は以下の通りです。
...
以下に初期スクリプトを示します。前述の通り、各 {{< gd-icon Node3D >}}`Node3D` をその局所座標系内で特定軸周りに回転させることに注意してください。
...
マウスとキーボードの制御を簡単に切り替えられるよう、`mouse_control`というフラグを追加してください。
