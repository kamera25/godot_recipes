...
{{% notice note %}}
クロス積は両方のベクトルに垂直な値となるため、この計算を行うには3次元空間で作業している必要があります。Godotを含むほとんどの2Dフレームワークでは、`Vector2.cross()` メソッドは結果の大きさを表すスカラー値を返します。
{{% /notice %}}
...
この例は、これらの手法の2つの典型的な応用事例を示しています。赤色ベクトルが対象物の前方方向を表し、緑色ベクトルが他の物体に向かう方向を示す場合：
